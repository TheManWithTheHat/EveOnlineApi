...
class EveAccountUpdateComponent extends Component {
	...
	public function update($user_id, $account_id = null, $keyID = null, $vCode = null, $options = array()) {
		...
		if($result) {
			...
			$paid = 0;
			if($this->Account->data['Account']['type'] == 'Account') {
				$this->characters = $this->EveOnlineApi->response['result']['key']['rowset']['row'];
				...
				if($result) {
					...
					$paid = strtotime($this->EveOnlineApi->response['result']['paidUntil']);
				} else {
					//Error Handling
			...
			$this->Account->data['Account']['lastApiVisit'] = date('Y-m-d H:i:s', time());
			if($this->Account->data = $this->Account->save($this->Account->data)) {
				if($paid > time() && ($this->Account->data['Account']['type'] == 'Account' || $this->Account->data['Account']['type'] == 'Character')) {
					$this->doCharacterImport();
				} else if($this->Account->data['Account']['type'] == 'Corporation') {
					$this->doCorporationImport($this->EveOnlineApi->response['result']['key']['rowset']['row']['@corporationID'], $this->EveOnlineApi->response['result']['key']['rowset']['row']['@corporationName']);	
				}
			} else {
				return $this->Account;
			}
	...
	private function doCharacterImport() {
		if(!empty($this->characters)) {
			if(!isset($this->characters[0])) {
				$this->characters = array(0 => $this->characters);
			}
	...
	private function doCorporationImport($corporationID, $corporationName) {
		App::import('Model', 'EveOnlineApi.Corporation');
		$this->Corporation = new Corporation();
		$this->Corporation->id = intval($corporationID);
		$this->Corporation->data = $this->Corporation->read();		
		$this->Corporation->data = array('Corporation' => array(
			'corporationID' => $corporationID, 
			'corporationName' => $corporationName,
			'lastApiUpdate' => date('Y-m-d H:i:s', time())
		));
		
		
		if($this->EveOnlineApi->validateAccessMask($this->accessMask, array('CorporationWalletJournal'))) {
//			$this->handleCorporationWalletJournal(isset($this->Corporation->data['Corporation']['lastWalletJournal']) ? $this->Corporation->data['Corporation']['lastWalletJournal'] : null);
		}
		if(!empty($this->Corporation->data)) {
			if(isset($this->Corporation->data['Corporation']['corporationID']) && !$this->Corporation->save($this->Corporation->data)) {
				throw new InternalErrorException();
			}
		}
	}
